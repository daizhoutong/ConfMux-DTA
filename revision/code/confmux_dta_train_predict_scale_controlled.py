...
# -*- coding: utf-8 -*-
"""
ConfMux-DTA leakage- and scale-controlled training and prediction pipeline.
...
Typical usage:
  python confmux_dta_train_predict_scale_controlled.py train_test \
      --data_file preprocessed/confmux_dta_bindingdb_agg.csv \
      --split_mode random \
      --train_fraction 0.5 \
      --reference_cache_dir confmux_dta_feature_cache/feat_train_only_633b5630a6107d71 \
      --total_iters 50000 \
      --stage_iters 2000
...
CONFIG: Dict = {
    ...
    "SEED": 5313,
    "SPLIT_MODE": "compound",
    # Fraction of the original training partition used to fit both the
    # representation and the regressor. The validation partition is unchanged.
    "TRAIN_FRACTION": 1.0,
    "TRAIN_SUBSAMPLE_SEED": 20260910,
    "REFERENCE_CACHE_DIR": "",
...
def _index_sha1(indices: np.ndarray) -> str:
    arr = np.ascontiguousarray(np.asarray(indices, dtype=np.int64))
    return hashlib.sha1(arr.tobytes()).hexdigest()


def verify_reference_split(reference_cache_dir: str, idx_tr: np.ndarray, idx_va: np.ndarray) -> Dict[str, Any]:
    """Require the newly reconstructed full split to equal the published full-data split."""
    ref = str(reference_cache_dir or "").strip()
    if not ref:
        return {
            "checked": False,
            "base_train_index_sha1": _index_sha1(idx_tr),
            "valid_index_sha1": _index_sha1(idx_va),
        }

    ref = os.path.abspath(ref)
    tr_path = os.path.join(ref, "idx_train.npy")
    va_path = os.path.join(ref, "idx_valid.npy")
    if not os.path.isfile(tr_path) or not os.path.isfile(va_path):
        raise FileNotFoundError(
            f"Reference cache must contain idx_train.npy and idx_valid.npy: {ref}"
        )
    ref_tr = np.asarray(np.load(tr_path), dtype=np.int64)
    ref_va = np.asarray(np.load(va_path), dtype=np.int64)
    if not np.array_equal(ref_tr, np.asarray(idx_tr, dtype=np.int64)):
        raise RuntimeError(
            "Reconstructed training indices do not exactly match the published 100% run"
        )
    if not np.array_equal(ref_va, np.asarray(idx_va, dtype=np.int64)):
        raise RuntimeError(
            "Reconstructed validation indices do not exactly match the published 100% run"
        )
    log(f"[Scale control] Exact reference split verified: {ref}")
    return {
        "checked": True,
        "reference_cache_dir": ref,
        "base_train_index_sha1": _index_sha1(ref_tr),
        "valid_index_sha1": _index_sha1(ref_va),
    }


def select_training_fraction(
    idx_tr_full: np.ndarray,
    df_all: pd.DataFrame,
    y_all: np.ndarray,
    fraction: float,
    seed: int,
) -> Tuple[np.ndarray, pd.DataFrame]:
    """Select a deterministic, endpoint- and response-stratified nested training subset."""
    fraction = float(fraction)
    if not (0.0 < fraction <= 1.0):
        raise ValueError("--train_fraction must be in the interval (0, 1]")

    idx_tr_full = np.asarray(idx_tr_full, dtype=np.int64)
    type_col = str(CONFIG.get("AFFINITY_UNIFIED", {}).get(
        "keep_type_col_name", "affinity_type"
    ))
    if type_col in df_all.columns:
        endpoint = df_all.iloc[idx_tr_full][type_col].astype(str).fillna("unknown").to_numpy()
    else:
        endpoint = np.repeat(str(CONFIG.get("AFFINITY", "pX")), len(idx_tr_full))

    if fraction >= 1.0:
        selected = idx_tr_full.copy()
    else:
        y_tr = np.asarray(y_all[idx_tr_full], dtype=np.float64)
        finite = np.isfinite(y_tr)
        if not np.all(finite):
            raise RuntimeError("Non-finite response encountered in the original training partition")
        # Ten global response bins crossed with endpoint identity preserve both
        # endpoint composition and the pX distribution in the nested subset.
        edges = np.unique(np.quantile(y_tr, np.linspace(0.0, 1.0, 11)))
        if len(edges) > 2:
            y_bin = np.digitize(y_tr, edges[1:-1], right=True).astype(str)
            strata = np.char.add(np.char.add(endpoint.astype(str), "::"), y_bin)
        else:
            strata = endpoint.astype(str)
    ...
    if np.setdiff1d(selected, idx_tr_full).size:
        raise RuntimeError("Training subset contains rows outside the original training partition")
    if len(np.unique(selected)) != len(selected):
        raise RuntimeError("Training subset contains duplicate indices")

    audit_rows = []
    for label, indices in (("base_train_100pct", idx_tr_full),
                           (f"selected_train_{fraction:.6f}", selected)):
        sub = df_all.iloc[indices]
        if type_col in sub.columns:
            groups = sub.groupby(type_col, dropna=False)
        else:
            groups = [(str(CONFIG.get("AFFINITY", "pX")), sub)]
        for affinity_type, part in groups:
            values = np.asarray(y_all[part.index.to_numpy(dtype=np.int64)], dtype=np.float64)
            audit_rows.append({
                "set": label,
                "affinity_type": str(affinity_type),
                "n": int(len(part)),
                "fraction_of_set": float(len(part) / max(1, len(sub))),
    ...
    log(
        f"[Scale control] selected={len(selected)} / base_train={len(idx_tr_full)} "
        f"({len(selected) / max(1, len(idx_tr_full)):.6f}); seed={int(seed)}"
    )
    return selected, audit


def _train_only_cache_tag(
    df_path: str,
    idx_tr: np.ndarray,
    idx_va: np.ndarray,
    idx_base_tr: Optional[np.ndarray] = None,
) -> str:
    """Fingerprint both representation settings and the exact split."""
    st = os.stat(df_path)
    meta = {
        "fit_scope": "selected_training_subset_only", "protocol_version": "scale-controlled-v2",
        ...
        "train_subsample_seed": int(CONFIG.get("TRAIN_SUBSAMPLE_SEED", 20260910)),
        "base_train_index_sha1": _index_sha1(idx_base_tr if idx_base_tr is not None else idx_tr),
...
def load_or_make_features_train_only(
    ...
    df_all: pd.DataFrame, y_raw: np.ndarray, idx_base_tr: Optional[np.ndarray] = None
) -> Tuple[sparse.csr_matrix, np.ndarray, np.ndarray, str]:
    """Fit on the selected training rows only; transform every other row."""
    print_step("Leakage/scale-controlled features", "fit=selected train only; validation=transform only")
    idx_base_tr = np.asarray(idx_base_tr if idx_base_tr is not None else idx_tr, dtype=np.int64)
    tag = _train_only_cache_tag(CONFIG["DATA_FILE"], idx_tr, idx_va, idx_base_tr)
    ...
    paths = {
        ...
        "ibt": os.path.join(cdir, "idx_base_train.npy"),
    }
    if (CONFIG["CACHE"]["enable"] and not CONFIG["CACHE"]["overwrite"]
        ...
        if meta.get("fit_scope") != "selected_training_subset_only":
            raise RuntimeError("Refusing cache without train-only fit provenance")
        if (not np.array_equal(np.load(paths["itr"]), idx_tr)
                or not np.array_equal(np.load(paths["iva"]), idx_va)
                or not np.array_equal(np.load(paths["ibt"]), idx_base_tr)):
    ...
    # Every non-fit row, including the fixed validation set and the unused half
    # of the original training partition, uses the frozen selected-train mapping.
    all_idx = np.arange(len(df_all), dtype=np.int64)
    idx_rest = np.setdiff1d(all_idx, idx_tr, assume_unique=False)
    L_rest = vect_lig.transform([lig_docs[i] for i in idx_rest])
    P_rest = vect_pr.transform([prot_docs[i] for i in idx_rest])
    if vec_type == "count" and vc.get("norm"):
        ...
        L_rest = sp_normalize(L_rest, norm=vc["norm"], copy=False)
        P_rest = sp_normalize(P_rest, norm=vc["norm"], copy=False)
    tr_blocks, rest_blocks = [L_tr, P_tr], [L_rest, P_rest]
    if use_affinity_type_feature():
        tr_blocks.append(affinity_type_onehot(df_all.iloc[idx_tr]))
        rest_blocks.append(affinity_type_onehot(df_all.iloc[idx_rest]))
    X_tr = sparse.hstack(tr_blocks, format="csr", dtype=np.float32)
    X_rest = sparse.hstack(rest_blocks, format="csr", dtype=np.float32)
    order = np.concatenate([idx_tr, idx_rest])
    stacked = sparse.vstack([X_tr, X_rest], format="csr")
    inverse = np.empty(len(df_all), dtype=np.int64)
    ...
    np.save(paths["itr"], idx_tr); np.save(paths["iva"], idx_va); np.save(paths["ibt"], idx_base_tr)
    joblib.dump(vect_lig, paths["vl"]); joblib.dump(vect_pr, paths["vp"])
    safe_json_dump({
        "tag": tag, "protocol_version": "scale-controlled-v2",
        "fit_scope": "selected_training_subset_only", "validation_scope": "transform_only",
        "split_mode": str(CONFIG["SPLIT_MODE"]), "test_size": float(CONFIG["TEST_SIZE"]),
        "seed": int(CONFIG["SEED"]), "n_train": int(len(idx_tr)), "n_valid": int(len(idx_va)),
        "n_base_train": int(len(idx_base_tr)),
        "train_fraction_requested": float(CONFIG.get("TRAIN_FRACTION", 1.0)),
        "train_fraction_realized": float(len(idx_tr) / max(1, len(idx_base_tr))),
        "train_subsample_seed": int(CONFIG.get("TRAIN_SUBSAMPLE_SEED", 20260910)),
        "base_train_index_sha1": _index_sha1(idx_base_tr),
        "train_index_sha1": hashlib.sha1(np.ascontiguousarray(idx_tr).tobytes()).hexdigest(),
        "valid_index_sha1": hashlib.sha1(np.ascontiguousarray(idx_va).tobytes()).hexdigest(),
...
def train_test_run(
    ...
    mask: np.ndarray,
    feat_cache_dir: str,
    idx_tr_override: Optional[np.ndarray] = None,
    idx_va_override: Optional[np.ndarray] = None,
) -> Dict:
    rs = int(CONFIG["SEED"])
    test_size = float(CONFIG["TEST_SIZE"])
    mode = (CONFIG.get("SPLIT_MODE") or "random").lower()
    ...
    if idx_tr_override is not None or idx_va_override is not None:
        if idx_tr_override is None or idx_va_override is None:
            raise ValueError("Both idx_tr_override and idx_va_override are required")
        idx_tr = np.asarray(idx_tr_override, dtype=np.int64)
        idx_va = np.asarray(idx_va_override, dtype=np.int64)
        if np.intersect1d(idx_tr, idx_va).size:
            raise RuntimeError("Scale-controlled training and validation indices overlap")
        if np.setdiff1d(np.concatenate([idx_tr, idx_va]), all_idx).size:
            raise RuntimeError("Scale-controlled indices are outside the prepared dataset")
        log(
            f"[Scale control] Using supplied indices: train={len(idx_tr)}; "
            f"fixed_validation={len(idx_va)}"
        )
    elif mode in ["compound", "protein", "pair"]:
    ...
    summary = {
        ...
        "config": {
            ...
            "TRAIN_FRACTION": float(CONFIG.get("TRAIN_FRACTION", 1.0)),
            "TRAIN_SUBSAMPLE_SEED": int(CONFIG.get("TRAIN_SUBSAMPLE_SEED", 20260910)),
            "train_index_sha1": _index_sha1(idx_tr),
            "valid_index_sha1": _index_sha1(idx_va),
        },
...
def build_arg_parser():
    ...
    def add_common_train_args(p):
        ...
        p.add_argument(
            "--train_fraction", type=float, default=float(CONFIG.get("TRAIN_FRACTION", 1.0)),
            help="Nested fraction of the original training partition retained for model fitting",
        )
        p.add_argument(
            "--train_subsample_seed", type=int,
            default=int(CONFIG.get("TRAIN_SUBSAMPLE_SEED", 20260910)),
            help="Random seed used only for the nested training-subset selection",
        )
        p.add_argument(
            "--reference_cache_dir", default=str(CONFIG.get("REFERENCE_CACHE_DIR", "")),
            help="Published 100%% train-only feature cache whose split indices must match exactly",
        )
...
def _apply_args_to_config(args):
    ...
    if getattr(args, "train_subsample_seed", None) is not None:
        CONFIG["TRAIN_SUBSAMPLE_SEED"] = int(args.train_subsample_seed)

    if getattr(args, "reference_cache_dir", None):
        CONFIG["REFERENCE_CACHE_DIR"] = os.path.abspath(str(args.reference_cache_dir))
...
def main():
    ...
    idx_tr_full, idx_va, df_all, y_raw = make_fixed_train_valid_split(df, mask)
    split_verification = verify_reference_split(
        str(CONFIG.get("REFERENCE_CACHE_DIR", "")), idx_tr_full, idx_va
    )
    idx_tr, fraction_audit = select_training_fraction(
        idx_tr_full,
        df_all,
        y_raw,
        float(CONFIG.get("TRAIN_FRACTION", 1.0)),
        int(CONFIG.get("TRAIN_SUBSAMPLE_SEED", 20260910)),
    )
    fraction_audit.to_csv(
        os.path.join(CONFIG["OUT_DIR"], "training_fraction_by_endpoint.csv"), index=False
    )
    fraction_manifest = {
        **split_verification,
        "data_file": os.path.abspath(CONFIG["DATA_FILE"]),
        ...
        "split_mode": str(CONFIG["SPLIT_MODE"]),
        "test_size": float(CONFIG["TEST_SIZE"]),
        "base_train_n": int(len(idx_tr_full)),
        "selected_train_n": int(len(idx_tr)),
        "valid_n": int(len(idx_va)),
        "train_fraction_requested": float(CONFIG.get("TRAIN_FRACTION", 1.0)),
        "train_fraction_realized": float(len(idx_tr) / max(1, len(idx_tr_full))),
        "train_subsample_seed": int(CONFIG.get("TRAIN_SUBSAMPLE_SEED", 20260910)),
        "selected_train_index_sha1": _index_sha1(idx_tr),
        "selected_is_nested_in_base_train": bool(
            np.setdiff1d(idx_tr, idx_tr_full).size == 0
        ),
    }
    safe_json_dump(
        fraction_manifest, os.path.join(CONFIG["OUT_DIR"], "training_fraction_manifest.json")
    )
    # Refresh the config snapshot after the validated split metadata are known.
    CONFIG["SCALE_CONTROL_MANIFEST"] = fraction_manifest
    safe_json_dump(CONFIG, os.path.join(CONFIG["OUT_DIR"], "config_used.json"))
    Xs, y_raw, w, feat_cache_dir = load_or_make_features_train_only(
        df, mask, idx_tr, idx_va, df_all, y_raw, idx_base_tr=idx_tr_full
    )
    ...
    if args.command == "train_test":
        train_test_run(
            ...
            mask=mask,
            feat_cache_dir=feat_cache_dir,
            idx_tr_override=idx_tr,
            idx_va_override=idx_va,
        )
