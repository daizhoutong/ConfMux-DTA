...
# -*- coding: utf-8 -*-
"""
ConfMux-DTA training and prediction pipeline.
...
The code is intended for academic review and reproducibility. It keeps the final
...
def expand_affinity_rows(df_wide: pd.DataFrame) -> Tuple[pd.DataFrame, Dict[str, Any]]:
    ...
    for t in type_order:
        ...
        ww = w_vals[t].loc[m].to_numpy(dtype=np.float32)
...
def load_or_make_features(df: pd.DataFrame, mask: np.ndarray) -> Tuple[sparse.csr_matrix, np.ndarray, np.ndarray, str]:
    ...
    if CONFIG.get("AFFINITY_UNIFIED", {}).get("enable", False):
        ...
        w = pd.to_numeric(dfv[wcol], errors="coerce").fillna(1.0).to_numpy(dtype=np.float32)
    ...
    meta = {
        ...
        "has_affinity_type_feature": CONFIG.get("AFFINITY_UNIFIED", {}).get("enable", False),
    }
...
def _train_one_fold(
    ...
    with tqdm(total=stages, desc=f"{run_tag} stages") as pbar:
        for st in range(1, stages + 1):
            t1 = time.time()
            booster = lgb.train(
                params_used, dtrain,
                num_boost_round=stage_iters,
                init_model=booster,
            ...
            iters_per_sec = float(stage_iters / max(dur, 1e-6))
...
def main():
    ...
    # features
    Xs, y_raw, w, feat_cache_dir = load_or_make_features(df, mask)
    ...
    if args.command == "kfold":
        ...
        kfold_run(Xs, y_raw, w, df=df, mask=mask, feat_cache_dir=feat_cache_dir, k=k)
        log(f"[Done] kfold finished. k={k}")
        return
